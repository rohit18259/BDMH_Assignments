...
clf1 = AdaBoostClassifier(base_estimator = DecisionTreeClassifier(max_depth = 30), n_estimators = 400)
clf2 = KNeighborsClassifier(n_neighbors=8)
clf3 = RandomForestClassifier(n_estimators=2100, class_weight = 'balanced',max_features = 'sqrt',bootstrap = True)
clf4 = LogisticRegression(max_iter = 1500)

""" Here we define clf1 as an Adaboost Classifier, clf2 as a KNN classifier, clf3 as a randomforest classifier, clf4 as a logistic regression classifier.
We will use clf1,clf2,clf3 as base estimators and clf4 as the final estimator in a stacking classifier. We will then use that stacking classifier to finally predict our output."""


classifier = StackingClassifier(estimators = [('ada',clf1), ('knn',clf2), ('rf',clf3)], final_estimator = clf4)
